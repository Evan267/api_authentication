...
function sendResetMail (user, dbToken, email, res) {
  ...
  const mailOptions = {
    ...
    html: `<a href="https://ndd.com/reset_password/${resetToken}">Cliquer ici pour changer de mot de passe</a>`,
  }
...
function sendConfirmMail (user, dbToken, res, next, execNext) {
  ...
  const mailOptions = {
    to: user.email,
    from: process.env.EMAIL,
    subject: 'Confirmation de votre adresse mail pour votre inscription au site TEST',
    text: 'Bienvenue, Votre compte sur le site ndd.com a bien ete creer.',
    html: `<strong>Bienvenue sur le site TEST,</strong><br/><p>Pour confirmer votre compte veuillez <a href="https://ndd.com/confirm_password/${confirmToken}">cliquer ici</a></p>`,
  }
  transporter.sendMail(mailOptions)
    .then(() => {
      if (execNext) {
